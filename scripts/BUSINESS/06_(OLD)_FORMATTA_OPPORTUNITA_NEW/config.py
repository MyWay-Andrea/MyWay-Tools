...
def trova_file(path_cartella, parametro):
    ...
    for file in file_trovati:
        if parametro == "Opportunità":
            if "opportunita" in file.name.lower():
                    file_chose = file
        elif parametro == "Storico":
            if "storico opportunita" in file.name.lower():
                    file_chose = file
